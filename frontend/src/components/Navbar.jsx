...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
      <Link to='/'>
        <img src={assets.logo_img} className='w-28 sm:w-36' alt="" />
      </Link>

      <ul className='hidden sm:flex gap-5 text-sm text-amber-700'>
      ...
      <div className='flex items-center gap-6'>
        <img src={assets.search_icon} className='w-4 sm:w-5 cursor-pointer' onClick={() => setShowSearch(!showSearch)} alt="" />

        <div className='group relative'>
          <img onClick={() => token ? null : navigate('/login')} src={assets.profile_icon} className='w-4 sm:w-5 cursor-pointer' alt="" />
          {
            token && 
            <div className='absolute hidden group-hover:block right-0 dropdown-menu pt-4 z-10'>
              <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-amber-50 text-gray-500 rounded-md'>
        ...
        <Link to='/cart' className='relative' >
          <img src={assets.cart_icon} className='w-4 sm:w-5 min-w-4' alt="" />
          <p className='absolute right-[-5px] top-[-5px] w-4 text-center bg-amber-600 text-white leading-4 rounded-full aspect-square text-[8px]'>{getCartCount()}</p>
        </Link>
